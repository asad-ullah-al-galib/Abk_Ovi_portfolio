...
import { Box, PieChart } from "lucide-react";
import { BentoGrid, type BentoItem } from "@/components/ui/bento-grid";
import { FadingDots } from "@/components/ui/background-snippets";

export const metadata: Metadata = {
  title: "Employee Relations | ABK OVI",
  description: "Supporting teams through communication, problem solving, and steady leadership.",
};

export default function DataSciencePage() {
  const dataSkills: BentoItem[] = ["Exploratory Data Analysis (EDA)", "NumPy & Pandas", "Data Visualization", "Matplotlib & Seaborn", "Statistical Analysis", "Data Preprocessing"].map((service) => ({
    title: service,
    description: `Proficient in ${service.toLowerCase()} for people operations and reporting support.`,
    icon: <PieChart className="size-5 text-theme-accent" />,
    status: "People Ops",
    colSpan: 1,
  }));
      ...
      <main className="pt-[88px]">
        ...
        <section className="relative border-b border-dashed border-neutral-300 dark:border-white/10 overflow-hidden bg-white/40 dark:bg-black/20">
          ...
          <div className="mx-auto max-w-site px-6 py-20 relative z-10 flex flex-col md:flex-row items-start gap-6 md:gap-10">
            <div className="flex size-16 md:size-20 shrink-0 items-center justify-center rounded-full border border-dotted border-lime-200 bg-lime-50 text-slate-700 dark:border-lime-500/20 dark:bg-lime-500/10 dark:text-lime-200">
              <Box className="size-8 md:size-10" />
            </div>
            <div>
              ...
              <p className="mt-4 text-lg font-medium text-neutral-600 dark:text-neutral-300 max-w-2xl leading-relaxed">
                Supporting teams with communication, coordination, and practical problem solving.
              </p>
            </div>
          </div>
        </section>
        ...
        <section className="mx-auto max-w-site border-x border-dashed border-neutral-900/15 px-6 py-16 dark:border-white/10">
          <div className="space-y-12">
            <div>
              <h2 className="font-display text-2xl font-black text-theme-dark dark:text-theme-light">Skills & Tools</h2>
              <div className="mt-8">
                <BentoGrid items={dataSkills} className="md:grid-cols-2" />
              </div>
            </div>
          </div>
        </section>
