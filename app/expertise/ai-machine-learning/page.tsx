...
export const metadata: Metadata = {
  title: "Compensation & Benefits | ABK OVI",
  description: "Managing benefits support, payroll coordination, and policy execution with care.",
};

export default function AIMachineLearningPage() {
  const toolsItems: BentoItem[] = ["Python", "SARIMAX", "XGBoost", "Decision Trees", "TensorFlow", "PyTorch"].map((service) => ({
    title: service,
    description: `Practical support and structured execution using ${service}.`,
    icon: <Cpu className="size-5 text-theme-accent" />,
    status: "Support",
    colSpan: 1,
  }));
      ...
      <main className="pt-[88px]">
        ...
        <section className="relative border-b border-dashed border-neutral-300 dark:border-white/10 overflow-hidden bg-white/40 dark:bg-black/20">
          ...
          <div className="mx-auto max-w-site px-6 py-20 relative z-10 flex flex-col md:flex-row items-start gap-6 md:gap-10">
            <div className="flex size-16 md:size-20 shrink-0 items-center justify-center rounded-full border border-dotted border-lime-200 bg-lime-50 text-slate-700 dark:border-lime-500/20 dark:bg-lime-500/10 dark:text-lime-200">
              <Terminal className="size-8 md:size-10" />
            </div>
            <div>
              ...
              <p className="mt-4 text-lg font-medium text-neutral-600 dark:text-neutral-300 max-w-2xl leading-relaxed">
                Coordinating payroll support, benefits administration, and related HR documentation.
              </p>
            </div>
          </div>
        </section>
        ...
        <section className="mx-auto max-w-site border-x border-dashed border-neutral-900/15 px-6 py-16 dark:border-white/10">
          <div className="space-y-12">
            <div>
              <h2 className="font-display text-2xl font-black text-theme-dark dark:text-theme-light">Tools & Methods</h2>
              <div className="mt-8">
                <BentoGrid items={toolsItems} className="md:grid-cols-2" />
              </div>
            </div>
          </div>
        </section>
